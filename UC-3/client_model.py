...
class ml_pipe:
	...
	def __init__(self, data, model_name, features_numeral, features, classifier, load=False, options='{"isRegression":false}'):
		...
		self.hv = HashingVectorizer(decode_error='ignore', n_features=self.metadata.n_features) # Hashing for bag of words
	...
	def create_inference_graph(self):
		...
		# predict
			self.readout = self.readout_ops(self.hidden_layers[-1], shape = [self.architecture[-1], self.n_classes], name = 'readout', keep_prob = 1.0)
